...
async function showAllCat(id = "") {
  if (id === "") {
    //TODO update here or in frontend
    const sql = `SELECT CATEGORY_ID, NAME, DESCRIPTION, COURSES FROM MCSC.CATEGORY`;
    return queryWP(sql, "Cat.js: Failed to fetch all categories", "All selected");
  } else {
    const sql = `SELECT CATEGORY_ID, NAME, DESCRIPTION FROM MCSC.CATEGORY WHERE NAME = :v1`;
    let db;
    try {
      db = await connection(); // Await connection
      const res = await db.execute(sql, { v1: id}); // Await execute
...
async function showCatWithCourse(id, f = true) {
  if (f) {
    const sql = `SELECT c.course_id AS "COURSE_ID", c.course_name AS "COURSE_NAME", c.course_description AS "COURSE_DESCRIPTION", r.review AS "REVIEWS", r.rating AS "RATINGS"
                FROM MCSC.Courses c
